...
@implementation VOSCoreDataStack
...
//        NSDictionary *options = @{NSMigratePersistentStoresAutomaticallyOption: @YES,                                         NSInferMappingModelAutomaticallyOption : @YES} ;
        ...
        if (![_storeCoordinator addPersistentStoreWithType:NSSQLiteStoreType
            ...
            NSNotification *note = [NSNotification
                                    notificationWithName:[VOSCoreDataStack persistentStoreCoordinatorErrorNotificationName]
                                    object:self
                                    userInfo:@{@"error" : err}];
...
-(void) saveWithErrorBlock: (void(^)(NSError *error))errorBlock{
    ...
    if (!_context) {
        err = [NSError errorWithDomain:@"VOSCoreDataStack"
                                  code:1
                              userInfo:@{NSLocalizedDescriptionKey :
               @"Attempted to save a nil NSManagedObjectContext. This VOSCoreDataStack has no context - probably there was an earlier error trying to access the CoreData database file."}];
        errorBlock(err);
